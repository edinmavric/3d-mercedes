...
export function EngineModel() {
  ...
  const normalized = useMemo(() => {
    ...
    // Smaller — the engine slot is small; previous 2.2 was bursting the frame.
  ...
  useFrame((_state, delta) => {
    ...
    // Slower auto-rotation; less aggressive accel from progress.
    g.rotation.y += delta * (0.18 + p * 0.35);
    const targetTiltX = THREE.MathUtils.lerp(-0.32, -0.08, p);
